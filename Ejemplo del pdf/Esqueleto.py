...
class AutocompleteApp:
    def __init__(self, root, trie):
        self.trie = trie
        self.root = root
        self.root.title("Buscador con Trie")
        ...
        # Listbox (tkinter puro, no ttkbootstrap)
        self.listbox = tk.Listbox(frame, width=50, height=6, font=("Helvetica", 12),
                                  bg="white", fg="#333", selectbackground="#cce5ff")
        self.listbox.pack(pady=10)

        # Botón buscar
        self.search_button = tb.Button(frame, text="Buscar", bootstyle="success-outline", command=self.confirm_search)
...
if __name__ == "__main__":
    trie = Trie()
    # Palabras iniciales
    for word in ["casa", "caso", "casta", "canasta", "cantar", "carro", "carta"]:
        trie.insert(word)

    app = tb.Window(themename="flatly")  # prueba "cyborg", "superhero", "darkly"
